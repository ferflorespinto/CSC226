/* MST.java
   ...
   Completed by Jorge Fernando Flores Pinto
   ID: V00880059
*/
...
public class MST {
    ...
    */
    static int mst(int[][] G) {
		...
		int[][] Gcopy = new int[G.length][G[0].length];

		/* Find a minimum spanning tree using Kruskal's algorithm */
		/* (You may add extra functions if necessary) */
			
		/* ... Your code here ... */
		EdgeWeightedGraph weightedG	= new EdgeWeightedGraph(numVerts, G);
		//PriorityQueue<Edge> edgeList = weightedG.adj;
		
		int totalWeight = 0;
		//for(Edge e : edgeList) {
		//while(!edgeList.isEmpty()) {
			//Edge f = edgeList.poll();
			//System.out.println("Vertex: " + f.either() + ", vertex: " + f.other(f.either()) + ", weight: " + f.weight());
			//totalWeight += f.weight();
		//}
		PriorityQueue<Edge> pqUncycled = weightedG.kruskalMST(G);

		//System.out.println("Finished Kruskal, now dequeueing: ");

		while(!pqUncycled.isEmpty()) {
			Edge f = pqUncycled.poll();
			//System.out.println("Vertex: " + f.either() + ", vertex: " + f.other(f.either()) + ", weight: " + f.weight());
			totalWeight += f.weight();
		}
		//System.out.println("Finished dequeueing");
